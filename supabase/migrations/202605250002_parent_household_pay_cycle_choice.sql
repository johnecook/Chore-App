-- Expand parent household onboarding so payout cadence is chosen, not assumed weekly.

drop function if exists public.create_parent_household(text, text, int);
...
create or replace function public.create_parent_household(
  ...
  pay_cycle public.pay_cycle_type default 'weekly',
...
begin
  ...
  if pay_cycle not in ('weekly', 'biweekly') then
    raise exception 'Onboarding supports weekly or biweekly payout schedules';
  end if;

  if pay_cycle = 'biweekly' and biweekly_anchor_date is null then
